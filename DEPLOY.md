...
**That automation now exists** — see `android-relay/README.md`. A phone-side
relay app forwards incoming MoMo SMS to `POST /api/webhooks/momo-sms`
(`server/momo-sms.ts`), which parses, matches by amount + clinic name in
the MoMo Reason field, and calls the exact same approval path as the admin
click above. Requires:
1. `MOMO_SMS_WEBHOOK_SECRET` set in Render env vars.
2. `pnpm db:push` to create `momoSmsEvents`.
3. The relay phone app installed and configured — see `android-relay/README.md`.
Anything the auto-approver can't confidently match lands in Owner Dashboard
→ Billing → **MoMo SMS Events** for manual review; the admin approve button
above remains the permanent fallback.
