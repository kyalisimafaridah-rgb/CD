export const ENV = {
  ...
  // Shared secret the phone-side MoMo SMS relay app (android-relay/) must
  // send as the X-Momo-Secret header on every forwarded SMS — see
  // server/momo-sms.ts and POST /api/webhooks/momo-sms below. Unset means
  // that endpoint always rejects, same fail-safe pattern as backupCronSecret.
  momoSmsWebhookSecret: process.env.MOMO_SMS_WEBHOOK_SECRET ?? "",
};
