...
import { processInboundMomoSms } from "../momo-sms";
...
async function startServer() {
  ...
  // ─── MTN/Airtel MoMo SMS relay (auto-approve subscription payments) ───────
  // The phone-side listener app (android-relay/) POSTs the raw SMS body here
  // whenever it sees a message from MTNMobMoney/AirtelMoney. Auth is a
  // shared secret header, not a signature scheme — MTN/Airtel don't sign
  // anything for us, the trust boundary is "only our own relay phone knows
  // MOMO_SMS_WEBHOOK_SECRET". Rotate it in Render env vars + the relay app's
  // settings screen if the phone is ever lost.
  app.post("/api/webhooks/momo-sms", async (req, res) => {
    const configuredSecret = ENV.momoSmsWebhookSecret;
    const providedSecret = req.headers["x-momo-secret"];

    if (!configuredSecret) {
      // Fail-safe: unconfigured means this endpoint always rejects, same
      // pattern as BACKUP_CRON_SECRET — never silently accept unauthenticated
      // requests just because an env var is missing.
      res.status(503).json({ error: "MoMo SMS webhook not configured" });
      return;
    }
    if (typeof providedSecret !== "string") {
      res.status(401).json({ error: "Missing X-Momo-Secret header" });
      return;
    }
    ...
    const senderRaw = String(req.body?.sender ?? "").toLowerCase();
    if (!body) {
      res.status(400).json({ error: "Missing body" });
      return;
    }
    const network: "mtn" | "airtel" = senderRaw.includes("airtel") ? "airtel" : "mtn";

    try {
      const outcome = await processInboundMomoSms(body, network);
      res.status(200).json({ received: true, outcome: outcome.status });
    } catch (error) {
      console.error("[MoMo SMS] Webhook handler error:", error);
      if (Sentry) Sentry.captureException(error);
      // Still 200 — the relay app has no useful retry story beyond "the
      // phone will see this SMS exactly once." A 500 here just means the
      // relay logs a failure with no way to recover the SMS; better to
      // accept receipt and rely on momoSmsEvents (or its absence, if the
      // failure happened before logging) for follow-up.
