 ...
 *
 * Why this exists: the admin's manual "approve" click and the MoMo SMS
 * auto-approve webhook both call applyPaymentApproval(), and both start
 * with a plain SELECT to check req.status !== "pending". That read-then-write
 * has a race window — two near-simultaneous calls (a relay retry racing an
 * admin click, or two webhook deliveries) can both read "pending" before
 * either writes "approved", double-applying the upgrade and double-logging
 * subscription events. This claim must be the first thing
 * applyPaymentApproval does; if it returns false, the caller must stop
 * before touching billing.
 */
export async function claimPendingPaymentRequest(id: number): Promise<boolean> {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db
    .update(subscriptionPaymentRequests)
    .set({ status: "approved" })
    .where(and(eq(subscriptionPaymentRequests.id, id), eq(subscriptionPaymentRequests.status, "pending")))
    .returning({ id: subscriptionPaymentRequests.id });
...
/**
 * Candidate pending requests for a given amount — the first cut of matching
 * an inbound MoMo SMS. Deliberately does NOT filter by reason text here;
 * that fuzzy comparison happens in server/momo-sms.ts where it can be
 * unit-tested against real SMS wording, not buried in a SQL LIKE.
 */
export async function listPendingPaymentRequestsByAmount(amountUgx: number) {
  const db = await getDb();
  if (!db) return [];
  return await db
    .select()
    .from(subscriptionPaymentRequests)
    .where(and(eq(subscriptionPaymentRequests.status, "pending"), eq(subscriptionPaymentRequests.amountUgx, amountUgx)));
}
...
// ===== MOMO SMS AUTO-APPROVE (server/momo-sms.ts) =====

/**
 * Records the outcome of one inbound MoMo SMS, whatever it was. Relies on
 * the unique index on transactionId to make double-processing impossible
 * at the DB layer, not just in application logic — insert throws if this
 * transactionId was already logged, caller (momo-sms.ts) catches that and
 * reports "duplicate" without touching any clinic's subscription.
 */
export async function createMomoSmsEvent(event: typeof momoSmsEvents.$inferInsert) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  const result = await db.insert(momoSmsEvents).values(event).returning({ id: momoSmsEvents.id });
  return result[0];
}

/**
 * Overwrites the outcome fields of a row already reserved by
 * createMomoSmsEvent. Used because the transactionId INSERT has to happen
 * BEFORE matching (it's the dedup guard against a race — see
 * processInboundMomoSms) but the actual status isn't known until after
 * matching runs, so the row is created with a placeholder status and
 * corrected here once the real outcome is known.
 */
export async function updateMomoSmsEventStatus(
  id: number,
  updates: Partial<Pick<typeof momoSmsEvents.$inferInsert, "status" | "matchedPaymentRequestId" | "matchedClinicId" | "note">>
) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db.update(momoSmsEvents).set(updates).where(eq(momoSmsEvents.id, id));
}

export async function getMomoSmsEventByTransactionId(transactionId: string) {
  const db = await getDb();
  if (!db) return undefined;
  const rows = await db.select().from(momoSmsEvents).where(eq(momoSmsEvents.transactionId, transactionId)).limit(1);
  return rows.length > 0 ? rows[0] : undefined;
}

export async function listRecentMomoSmsEvents(limit = 100) {
  const db = await getDb();
  if (!db) return [];
  return await db
    .select({
      id: momoSmsEvents.id,
      network: momoSmsEvents.network,
      transactionId: momoSmsEvents.transactionId,
      parsedAmountUgx: momoSmsEvents.parsedAmountUgx,
      parsedReasonName: momoSmsEvents.parsedReasonName,
      parsedReasonPhone: momoSmsEvents.parsedReasonPhone,
      status: momoSmsEvents.status,
      matchedPaymentRequestId: momoSmsEvents.matchedPaymentRequestId,
      matchedClinicId: momoSmsEvents.matchedClinicId,
      note: momoSmsEvents.note,
      resolvedAt: momoSmsEvents.resolvedAt,
      createdAt: momoSmsEvents.createdAt,
      clinicName: clinics.name,
    })
    .from(momoSmsEvents)
    .leftJoin(clinics, eq(momoSmsEvents.matchedClinicId, clinics.id))
    .orderBy(desc(momoSmsEvents.createdAt))
    .limit(limit);
}

export async function resolveMomoSmsEvent(id: number, userId: number, note?: string) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
  return await db
    .update(momoSmsEvents)
    .set({ resolvedAt: new Date(), resolvedByUserId: userId, note: note ?? undefined })
    .where(eq(momoSmsEvents.id, id));
}
