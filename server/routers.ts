...
import { publicProcedure, router, protectedProcedure, adminProcedure, rateLimited } from "./_core/trpc";
import { z } from "zod";
...
import { applyPaymentApproval } from "./momo-sms";
...
<p><a href="${ENV.appUrl}/settings" style="background:#2563eb;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;display:inline-block">Upgrade to Clinic (UGX 90,000/mo)</a></p>`,
  ...
  admin: router({
    ...
    getAllClinics: adminProcedure.query(() =>
      db.getAllClinicsWithStats()
    ),
    getClinicStaff: adminProcedure
      .input(z.object({ clinicId: z.number() }))
      .query(async ({ input }) => {
    ...
    updateClinicTier: adminProcedure.input(z.object({
      clinicId: z.number(),
    ...
    getBillingIssues: adminProcedure.query(() => db.getSubscriptionEventsNeedingReview()),

    resolveBillingIssue: adminProcedure
      .input(z.object({ id: z.number() }))
    ...
    getAuditLog: adminProcedure.query(() => db.getAdminAuditLog()),
    ...
    listPaymentRequests: adminProcedure.query(() => db.listRecentPaymentRequests(100)),

    listPendingPaymentRequests: adminProcedure.query(() => db.listPendingPaymentRequests(100)),
    ...
    approvePaymentRequest: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        // Shared with the MoMo SMS auto-approve path (server/momo-sms.ts) —
        // one implementation of "what happens when a payment is confirmed",
        // whether a human clicked or the SMS listener matched it.
        const result = await applyPaymentApproval({
          paymentRequestId: input.id,
          reviewedByUserId: ctx.user.id,
          reviewNote: input.reviewNote?.trim() || "",
          subscriptionEventNote: `Payment request #${req.id} approved (MTN MoMo self-service) — ${req.durationMonths} month(s)`,
          activityAction: "ADMIN_APPROVE_PAYMENT_REQUEST",
        });
        return { success: true as const, tier: result.tier, appliedUntil: result.appliedUntil };
      }),
    ...
    // Review queue for the MoMo SMS auto-approve pipeline (server/momo-sms.ts).
    // "matched" rows already went through and upgraded the clinic — shown
    // here for audit only. "no_match" / "ambiguous" / "duplicate" /
    // "parse_failed" rows need a human: either approve the underlying
    // request manually above, or investigate why the SMS didn't match.
    listMomoSmsEvents: adminProcedure.query(() => db.listRecentMomoSmsEvents(100)),

    resolveMomoSmsEvent: adminProcedure
      .input(z.object({ id: z.number(), note: z.string().max(500).optional() }))
      .mutation(async ({ ctx, input }) => {
        await db.resolveMomoSmsEvent(input.id, ctx.user.id, input.note?.trim());
        return { success: true } as const;
      }),

generateActivationCode: adminProcedure
      .input(z.object({
        tier: z.enum(["clinic", "pro"]),
        durationMonths: z.number().int().min(1).max(36),
    ...
    listActivationCodes: adminProcedure.query(() => db.listActivationCodes(150)),

    revokeActivationCode: adminProcedure
      .input(z.object({ id: z.number() }))
      .mutation(async ({ ctx, input }) => {
