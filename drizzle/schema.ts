...
export const momoSmsEventStatusEnum = pgEnum("momo_sms_event_status", [
  "matched",       // parsed cleanly, matched exactly one pending request, auto-approved
  "no_match",      // parsed cleanly, but no pending request had that amount
  "ambiguous",     // parsed cleanly, but more than one pending request shared that amount
  "duplicate",     // this MoMo transaction ID was already processed
  "parse_failed",  // raw body didn't match any known MTN/Airtel format
]);

/**
 * Raw log of every inbound MoMo SMS relayed from the phone-side listener,
 * regardless of outcome. This is the audit trail + admin review queue for
 * the auto-approve pipeline (server/momo-sms.ts) — every row we couldn't
 * confidently auto-approve should be visible here, not just console.warn'd,
 * same reasoning as subscriptionEvents.needsReview for the Lemonsqueezy path.
 */
export const momoSmsEvents = pgTable(
  "momoSmsEvents",
  {
    id: serial("id").primaryKey(),
    /** Raw SMS body exactly as relayed by the phone listener app */
    rawBody: text("rawBody").notNull(),
    /** "mtn" | "airtel" — which sender the relay app matched on */
    network: varchar("network", { length: 20 }),
    /** MTN/Airtel's own transaction ID, parsed from the SMS. Unique so a
     *  duplicate relay (phone retry, listener re-triggering) can never be
     *  double-applied to a clinic's subscription. */
    transactionId: varchar("transactionId", { length: 64 }),
    parsedAmountUgx: integer("parsedAmountUgx"),
    parsedReasonName: varchar("parsedReasonName", { length: 160 }),
    parsedReasonPhone: varchar("parsedReasonPhone", { length: 30 }),
    status: momoSmsEventStatusEnum("status").notNull(),
    /** Set when status = "matched" */
    matchedPaymentRequestId: integer("matchedPaymentRequestId"),
    matchedClinicId: integer("matchedClinicId"),
    /** Freeform note — e.g. which candidate clinic names tied on "ambiguous" */
    note: text("note"),
    resolvedAt: timestamp("resolvedAt"),
    resolvedByUserId: integer("resolvedByUserId"),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
  ...
  (table) => ({
    transactionIdUnique: uniqueIndex("momoSmsEvents_transactionId_unique").on(table.transactionId),
  })
);

export type MomoSmsEvent = typeof momoSmsEvents.$inferSelect;
export type InsertMomoSmsEvent = typeof momoSmsEvents.$inferInsert;
